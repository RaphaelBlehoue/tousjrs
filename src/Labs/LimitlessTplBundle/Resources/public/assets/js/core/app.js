...
$(function() {
    ...
    $(document).on('keyup', '.flash', function (e) {

        var $elt = $(this);
        var nombreCaractere = $elt.val().length;
        var nombreCaractere = 160 - nombreCaractere;

        var nombreMots = jQuery.trim($elt.val()).split(' ').length;
        if($elt.val() === '') {
            nombreMots = 0;
        }

        var msg2 = ' ' + nombreMots + ' mot(s) | ' + nombreCaractere + ' Caractere(s) restant';
        $('#compteur').text(msg2);
        if (nombreCaractere < 1) {
            $('.compteur').addClass("has-error");
            $('#compteur').addClass("text-danger-700");
        } else {
            $('.compteur').removeClass("has-error");
            $('#compteur').removeClass("text-danger-700");
        }
